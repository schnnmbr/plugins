...
if ( ! class_exists( 'Themify_Builder_Options' ) ) {
	...
	class Themify_Builder_Options {
		...
		public function add_plugin_page(){
			...
			add_menu_page( 'Themify Builder', 'Themify Builder', 'manage_options', self::$slug, array( $this, 'create_admin_page'), plugins_url( 'themify-builder/themify/img/favicon.png' ) );
		}
		...
		public function page_init() {		
			...
			switch ( $current_tab ) {
				case 'image_setting':
					...
					add_settings_field(
						'image_quality', 
						__('Image Quality', 'themify'), 
						array( $this, 'image_quality_field' ), 
						self::$slug,
						'setting_builder_image_section'			
					);
					...
					add_settings_field(
						'image_crop_align', 
						__('Cropping Alignment', 'themify'), 
						array( $this, 'image_crop_align_field' ), 
						self::$slug,
						'setting_builder_image_section'			
					);
					...
					add_settings_field(
						'image_vertical_crop', 
						__('Vertical Cropping', 'themify'), 
						array( $this, 'image_vertical_crop_field' ), 
						self::$slug,
						'setting_builder_image_section'			
					);
				break;
		...
		function builder_exclude_field() {
			...
			<label for="exclude_module_<?php echo $v['name']; ?>"><?php echo sprintf(__('Module %s', 'themify'), ucfirst($v['name'])); ?></label>
			</p>
			<?php endforeach; ?>
			<?php
		}
		...
		function image_script_field() {
			...
			echo '<label for="themify_setting-img_settings_use">&nbsp; '.__('Disable img.php globally','themify').'</label>';

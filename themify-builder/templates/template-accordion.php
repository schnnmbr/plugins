...
<div id="<?php echo esc_attr( $module_ID ); ?>" class="<?php echo esc_attr( $container_class ); ?>" data-behavior="<?php echo esc_attr( $expand_collapse_accordion ); ?>">
	...
	<ul class="<?php echo esc_attr( $ui_class ); ?>">
		<?php foreach ( array_filter( $content_accordion ) as $content ):
		...
		<li>
			...
			<div class="accordion-content <?php echo ( ( ! empty( $content['default_accordion'] ) && $content['default_accordion'] != 'open' ) || empty( $content['default_accordion'] ) ) ? 'default-closed' : ''; ?> clearfix">
				<?php

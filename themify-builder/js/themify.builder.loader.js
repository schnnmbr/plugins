...
(function($, window, document, undefined){
	...
	$(document).ready(function(){
		...
		$('body').on('click.tbloader', '.toggle_tf_builder a:first, a.themify_builder_turn_on', function(e){
		...
		if ( window.location.hash === "#builder_active" ) {
			...
			} else if ( $('.themify_builder_turn_on').length > 0 ) {
				$('.themify_builder_turn_on').first().trigger( 'click' );
			}
		}

...
function twentig_ajax_customize_load_starter_content() {
	...
	if ( isset( $content['theme_mods'] ) ) {
		foreach ( $wp_customize->settings() as $setting ) {
			...
			if ( in_array( $setting->id, array( 'custom_logo', 'display_title_and_tagline', 'background_color', 'background_image', 'display_excerpt_or_full_post', 'respect_user_color_preference' ), true ) && ! isset( $content['theme_mods'][ $setting->id ] ) ) {
				$content['theme_mods'][ $setting->id ] = $setting->default;
			}
		}
	}

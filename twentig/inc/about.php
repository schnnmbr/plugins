...
function twentig_menu_page() {
	...
	add_submenu_page(
		'twentig',
		esc_html__( 'Starter Websites', 'twentig' ),
		esc_html__( 'Starter Websites', 'twentig' ),
		'manage_options',
		esc_url( admin_url( 'customize.php?autofocus[section]=twentig_starter_websites' ) . '&return=' . rawurlencode( admin_url() . '?page=twentig' ) )
	);
	...
	<div class="tw-about__container">
		<div class="tw-about__header">
			<nav class="tw-about__header-navigation" aria-label="<?php esc_attr_e( 'Primary menu' ); ?>">
				...
				<a href="<?php echo esc_url( 'https://wordpress.org/support/plugin/twentig/reviews/?filter=5' ); ?>" target="_blank"><?php esc_html_e( 'Rate Twentig', 'twentig' ); ?></a>
		...
		<div class="tw-about__section is-feature">
			<div class="tw-about__section-inner">
				...
				<?php $text = esc_html__( 'From advanced Twenty Twenty-One options to enhanced blocks to pre‑designed layouts, you’ve got what you need to create a beautiful website. Start from scratch or choose one of our starter websites.', 'twentig' ); ?>
				<p>
					<?php
					if ( 'twentytwenty' === $theme ) {
						echo str_replace( 'Twenty Twenty-One', 'Twenty Twenty', $text );
					} else {
						echo $text;
					}
					?>
				...
				<p>
					<a href="<?php echo esc_url( admin_url( 'customize.php?autofocus[section]=twentig_starter_websites' ) . '&return=' . rawurlencode( admin_url() . '?page=twentig' ) ); ?>" class="button button-primary"><?php esc_html_e( 'Import starter website', 'twentig' ); ?></a>	
					<a href="https://twentig.com/demos?utm_source=wp-dash&utm_medium=overview-cta&utm_campaign=demos" class="button button-primary button-outline" target="_blank"><?php esc_html_e( 'View starter websites', 'twentig' ); ?></a>
				</p>
			</div>
		</div>
		<div class="tw-about__section ">
			<div class="tw-about__section-inner">
				...
				<?php $text = esc_html__( 'Twentig offers powerful features to customize Twenty Twenty-One — making it the perfect combination to build your website.', 'twentig' ); ?>
				<p>
					<?php
					if ( 'twentytwenty' === $theme ) {
						echo str_replace( 'Twenty Twenty-One', 'Twenty Twenty', $text );
				...
				<?php
				if ( 'twentytwenty' !== $theme && 'twentytwentyone' !== $theme && current_user_can( 'switch_themes' ) ) {
					...
					if ( in_array( 'twentytwentyone', array_keys( $installed_themes ), true ) ) {
						...
						?>
						<p><?php printf( '<a href="%s" class="button button-primary">%s</a>', esc_url( $install_url ), esc_html__( 'Activate Twenty Twenty-One', 'twentig' ) ); ?></p>
						<?php
					} else {
						$install_url = admin_url( 'theme-install.php?search=twentytwentyone' );
						?>
						<p><?php printf( '<a href="%s" class="button button-primary">%s</a>', esc_url( $install_url ), esc_html__( 'Install Twenty Twenty-One', 'twentig' ) ); ?></p>
						<?php
					}
				}
				?>
				...
				<div class="tw-about__columns has-3-columns">
					<div class="column">
						...
						<p>
							<?php esc_html_e( 'From post grid to sticky header to footer layout, Twentig provides endless ways to enhance your theme. Change the look and feel of your website by customizing the fonts, colors, global styles, 404 page, and more.', 'twentig' ); ?>
							<?php if ( 'twentytwenty' === $theme || 'twentytwentyone' === $theme ) : ?>
					...
					<div class="column">
						...
						<p><?php esc_html_e( 'Twentig brings design improvements to the default WordPress theme. Our additional block settings and pre-designed layouts are specially built for it — so it’s easier than ever to make beautiful pages.', 'twentig' ); ?></p>
					</div>					
				</div>	
			</div>
		</div>

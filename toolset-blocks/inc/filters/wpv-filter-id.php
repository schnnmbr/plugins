	...
	*/
	...
	static function wpv_add_filter_post_id_list_item( $view_settings ) {
		if ( isset( $view_settings['id_mode'][0] ) ) {
			...
			/**
			 * Filters the title of the Query Filter.
			 *
			 * @param string $title
	...
	*/
	...
	static function wpv_get_list_item_ui_post_id( $view_settings = array() ) {
		...
		<p class='wpv-filter-post-id-edit-summary js-wpv-filter-summary js-wpv-filter-post-id-summary'>
			<?php
			/**
			 * Filters the summary of the Query Filter.
			 *
			 * @param string $summary
			 * @param string $filter_type
			 * @param array  $view_settings
			 */
			echo apply_filters( 'wpv_filter_query_filter_summary', wpv_get_filter_post_id_summary_txt( $view_settings ), self::FILTER_TYPE, $view_settings );
			?>
		</p>
		<?php
		/**
		 * Filters whether the filter the filter title controls will be rendered or not.
		 *
		 * @param bool   $hide_filter_title_controls
		 * @param string $filter_type
		 * @param array  $view_settings
		 */
		$maybe_hide_filter_title_controls = apply_filters( 'wpv_filter_maybe_hide_filter_title_controls', false, self::FILTER_TYPE, $view_settings );
		if ( ! $maybe_hide_filter_title_controls ) {

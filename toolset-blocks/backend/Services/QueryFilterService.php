...
class QueryFilterService {
	const POST_RELATIONSHIP_FILTER_AUTOGENERATED_KEY = 'post_relationship_filter_autogenerated';
	...
	public function init() {
		...
		// It's safe for this to be run in priority 10, as the filter that add the relationship filter in the list, is
		// run at priority 1.
		add_filter( 'wpv_filters_add_filter', array( $this, 'maybe_remove_offered_post_relationship_filter' ), 10, 3 );
	}

	/**
	 * Removes the Post Relationship filter, when there is already an auto-generated one on the list of used query filters.
	 *
	 * Used when populating the Query Filter select to choose and create a new filter for the View.
	 *
	 * @param array  $filters
	 ...
	 * @return array
	 */
	public function maybe_remove_offered_post_relationship_filter( $filters, $post_type, $view_settings ) {
		if ( toolset_getarr( $view_settings, self::POST_RELATIONSHIP_FILTER_AUTOGENERATED_KEY, false ) ) {
			unset( $filters['post_relationship'] );
		}
		return $filters;
	}
	...
	public function maybe_adjust_view_settings_for_query_filter_injection( $view_settings, $view_id = null ) {
		...
		if (
			...
			$view_settings[ self::POST_RELATIONSHIP_FILTER_AUTOGENERATED_KEY ] = true;
		}

...
array(
		...
		array(
				...
				'message_for_link' => sprintf( __('Visit','woocommerce_views').' '.'%s<strong>'.__('WooCommerce Blocks documentation','woocommerce_views').'</strong>%s'.' '.__('to learn how to develop your own custom WooCommerce sites with Views.', 'woocommerce_views'), '<a href="' . WCVIEWS_TOPHELP_BOX_LINK . '" target="_blank">', '</a>' ),
		)
);
...
function wpv_wcviews_editing_instructions( $return, $section ) {
	if ( 'woocommerce-views-shortcodes' == $section ) {
			$return = array(
				...
				'title' => __( 'WooCommerce Blocks Shortcodes', 'woocommerce_views' ),
				'content' => '',
				'table' => array(
...
http://docs.woothemes.com/document/related-products-up-sells-and-cross-sells/', 'woocommerce_views' )
	...
	if ( 'woocommerce-views-functions' == $section ) {
		$return = array(
				...
				'title' => __( 'WooCommerce Blocks Public Functions', 'woocommerce_views' ),
				'content' => '',
				'table' => array(
						array(
						//woo_product_on_sale()

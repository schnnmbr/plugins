...
new WPUpdatesPluginUpdater_255( 'http://wp-updates.com/api/2/plugin', plugin_basename(__FILE__) );
*/

if( !class_exists('WPUpdatesPluginUpdater_255') ) {
    class WPUpdatesPluginUpdater_255 {
 ...
 //   		var_dump($raw_response);
    		...
    		if( !is_wp_error($raw_response) && ($raw_response['response']['code'] == 200) )
    			$this->response = unserialize($raw_response['body']);
    		
    		if( is_object($this->response) && !empty($this->response) ) // Feed the update data into WP updater
    			$transient->response[$this->plugin_path] = $this->response;
